...
[https://github.com/madisonliu2026/Liu_Madison_201_A06](https://github.com/madisonliu2026/Liu_Madison_201_A06)
...
```{r}
ppg = list()
for (player in names(player_info)){ #names() function
  pts = player_info[[player]]$points #index into player_info (b/c outer list isn't named), then index inner list by names
  games = player_info[[player]]$games_played
...
```{r}
rounded_ppg = sapply(ppg, round, digits = 2)
rounded_ppg
```
...
rounded_mat_rand = apply(mat_rand, MARGIN = 1:2, function(x) rounding_fun(x))
rounded_mat_rand
```
...
```{r}
avg_temp = apply(max_temps[-c(1,2)], 2, mean) #first two columns are not temperature data
avg_temp
```
### Part b (2 points)
...
```{r}
median_annual_temps = apply(colorado_temps[-c(1,2)], 1, median)
high_temp = max(median_annual_temps)
high = which.max(median_annual_temps)
high_year = colorado_temps[high, 2]
low_temp = min(median_annual_temps)
low = which.min(median_annual_temps)
low_year = colorado_temps[low, 2]

paste0("The maximum median annual temperature of ", high_temp, " occurred in ", high_year, " and the minimum median annual temperature of ", low_temp, " occurred in ", low_year, ".")
```
...
```{r}
summed_digits = function(x){
  #base case is then x is one digit and them sum is just that number
  if (x<10){
    return(x)
  }else{ #recurssive case is when x is more than 1 digit
    last_digit = x %% 10
    remaining_digits = x %/% 10
    return(last_digit + summed_digits(remaining_digits))
  }
}

result = summed_digits(935068)
result
